...
static const char* vmeconductor_spec[] =
{
    "implementation_id", "VMEConductor",
    "type_name",         "VMEConductor",
    "description",       "VMEConductor component",
    "version",           "1.0",
...
VMEConductor::VMEConductor(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("vmeconductor_out", m_out_data),
      m_recv_byte_size(0),
...
int VMEConductor::daq_configure()
{
    ...
    auto contClassGen = reinterpret_cast<ControllerCreateFnc*>(dlsym(contHandle, "MakeContObj"));
    error_s = dlerror();
    if(error_s != NULL)
    ...
    for(int i = 0; i<nrMods; i+=2){
        ...
        auto devClassGen = reinterpret_cast<DeviceCreateFnc*>(dlsym(modHandle, "MakeDevObj"));
        error_s = dlerror();
        if(error_s != NULL)
        ...
        vmeCont = vmeDev[i/2]->mod_configure(std::move(vmeCont), modAddr_v[i/2], (i/2));
...
int VMEConductor::read_data_from_detectors()
{
    ...
    for(int iter = 0; iter<nrMods; iter+=2){
        ...
        if(vmeDev[iter/2]->mod_checkifreader() == 1){
            ...
            vmeCont = vmeDev[iter/2]->mod_run(std::move(vmeCont), t_data_v, &fNEvents);
