...
private:
  ...
  typedef std::shared_ptr<TreeData> Hit_t;
  typedef std::vector<Hit_t> RawDataVec_t;
  RawDataVec_t fRawData; // vector is faster than deque for our purpose
  ...
  void WriteFile();
  bool fStopFlag;
  std::thread fMakePacketThread;
  bool fMakePacketFlag;
  std::thread fMakeTreeThread;
  bool fMakeTreeFlag;
  std::thread fWriteFileThread;
  std::deque<std::shared_ptr<RawDataVec_t>> fPacketQueue;
  std::deque<std::shared_ptr<TTree>> fTreeQueue;
  std::mutex fMutex;
};
