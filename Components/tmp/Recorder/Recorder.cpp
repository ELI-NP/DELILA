...
int Recorder::daq_start()
{
  ...
  fStopFlag = false;
  fMakePacketThread = std::thread(&Recorder::MakePacket, this);
...
int Recorder::FillData(unsigned int dataSize)
{
  ...
  for(unsigned int i = headerSize; i < dataSize + headerSize;) {
    auto hit = std::make_shared<TreeData>();

    // The order of hit should be the same as Reader
    memcpy(&(hit->Mod), &m_in_data.data[i], sizeMod);
    i += sizeMod;

    memcpy(&(hit->Ch), &m_in_data.data[i], sizeCh);
    i += sizeCh;

    memcpy(&(hit->TimeStamp), &m_in_data.data[i], sizeTS);
    i += sizeTS;

    memcpy(&(hit->FineTS), &m_in_data.data[i], sizeFineTS);
    i += sizeFineTS;

    memcpy(&(hit->ChargeLong), &m_in_data.data[i], sizeChargeLong);
    i += sizeChargeLong;

    memcpy(&(hit->ChargeShort), &m_in_data.data[i], sizeChargeShort);
    i += sizeChargeShort;

    memcpy(&(hit->RecordLength), &m_in_data.data[i], sizeRL);
    i += sizeRL;

    if(hit->RecordLength > 0){
      auto sizeTrace = sizeof((TreeData::Trace1[0])) * hit->RecordLength;
      hit->Trace1.resize(hit->RecordLength);
      memcpy(&hit->Trace1[0], &m_in_data.data[i], sizeTrace);
      i += sizeTrace;
    }
    
    {
      std::lock_guard<std::mutex> lock(fMutex);
      fRawData.push_back(hit);
    }
...
void Recorder::MakePacket()
{
  fMakePacketFlag = true;
  const auto limit = fDataLimit * 1.2;
  const auto oneHitSize = TreeData::OneHitSize;
  const auto nHits = (fDataLimit / oneHitSize) + 1;
  
  while(true){
    auto now = time(nullptr);
    if((now - fLastSave > fSaveInterval) ||
       (fDataSize > limit) || (fDataSize > 0 && fStopFlag)){

      auto packet = std::make_shared<RawDataVec_t>();
      if(fDataSize > fDataLimit) packet->resize(nHits);
      else packet->resize(fRawData.size());
      
      std::lock_guard<std::mutex> lock(fMutex);
      std::sort(fRawData.begin(), fRawData.end(), [](const Hit_t &a, const Hit_t &b){
	return a->FineTS < b->FineTS;
      });

      auto start = fRawData.begin();
      auto stop = fRawData.end();
      if(fRawData.size() > nHits)
	stop = start + nHits;

      std::copy(start, stop, packet->begin());

      fRawData.erase(start, stop);
      
      fLastSave = now;
      fDataSize = 0.;

      fPacketQueue.push_back(packet);
    }

    usleep(10000);

    if(fRawData.size() == 0 && fStopFlag)
      break;
  }

  fMakePacketFlag = false;
}
...
void Recorder::MakeTree()
{
  ...
  while(true){
    fMutex.lock();
    auto nEntries = fPacketQueue.size();
    fMutex.unlock();

    if(nEntries > 0) {
      fMutex.lock();
      auto dataVec = fPacketQueue.front();
      fPacketQueue.pop_front();

      auto tree = std::make_shared<TTree>("ELIADE_Tree", "ELIADE data");
	...
	// dataVec->at(iEve).reset(nullptr);
      ...
      // dataVec.reset(nullptr);
    }
    
    if(fPacketQueue.size() == 0 && fStopFlag && fMakePacketFlag == false)
      break;
...
void Recorder::WriteFile()
{
  while(true) {
    ...
    if(fTreeQueue.size() == 0 && fStopFlag && fMakeTreeFlag == false)
      break;
