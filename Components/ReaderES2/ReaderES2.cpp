...
static const char* reader_spec[] =
  {
   "implementation_id", "ReaderES2",
   "type_name",         "ReaderES2",
   "description",       "ReaderES2 component",
   "version",           "1.0",
...
ReaderES2::ReaderES2(RTC::Manager* manager)
...
RTC::ReturnCode_t ReaderES2::onInitialize()
{
  if (m_debug) {
    std::cerr << "ReaderES2::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t ReaderES2::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int ReaderES2::daq_dummy()
{
  return 0;
}

int ReaderES2::daq_configure()
{
  std::cerr << "*** ReaderES2::configure" << std::endl;
...
int ReaderES2::parse_params(::NVList* list)
{
...
int ReaderES2::daq_unconfigure()
{
  std::cerr << "*** ReaderES2::unconfigure" << std::endl;
...
int ReaderES2::daq_start()
{
  std::cerr << "*** ReaderES2::start" << std::endl;
...
int ReaderES2::daq_stop()
{
  std::cerr << "*** ReaderES2::stop" << std::endl;
...
int ReaderES2::daq_pause()
{
  std::cerr << "*** ReaderES2::pause" << std::endl;

  return 0;
}

int ReaderES2::daq_resume()
{
  std::cerr << "*** ReaderES2::resume" << std::endl;

  return 0;
}

int ReaderES2::read_data_from_detectors()
{
...
int ReaderES2::set_data(unsigned int data_byte_size)
{
...
int ReaderES2::daq_run()
{
  if (m_debug) {
    std::cerr << "*** ReaderES2::run" << std::endl;
  }
...
extern "C"
{
  void ReaderES2Init(RTC::Manager* manager)
  {
    RTC::Properties profile(reader_spec);
    manager->registerFactory(profile,
			     RTC::Create<ReaderES2>,
			     RTC::Delete<ReaderES2>);
  }
};
