...
int Recorder::daq_start()
{
  ...
  if (fDataWriteFlag) {
    ...
    for (auto &th : fWriteFileThreads) {
      th = std::thread(&Recorder::WriteFile, this);
    }
  }
...
{
  fpDataVec.reset(new std::vector<TraceData>);
  fpDataVec->reserve(1.4 * (fDataLimit / sizeof(TraceData)));
}
...
int Recorder::FillData(unsigned int dataSize)
{
  ...
  for (unsigned int i = headerSize; i < dataSize + headerSize;) {
    ...
    memcpy(&(data.ADC), &m_in_data.data[i], sizeADC);
    i += sizeADC;
...
void Recorder::MakeTree()
{
  ...
  while (true) {
    ...
    if (nEntries > 0) {
      ...
      auto tree = new TTree("Trace_Tree", "Trace data");
      ...
      tree->Branch("RecordLength", &RecordLength, "RecordLength/s");
