...
static const char* reader_spec[] =
  {
   "implementation_id", "ReaderES7",
   "type_name",         "ReaderES7",
   "description",       "ReaderES7 component",
   "version",           "1.0",
...
ReaderES7::ReaderES7(RTC::Manager* manager)
    ...
    m_debug(false)
{
  ...
  // Set OutPort buffers
  registerOutPort("reader_es7_out", m_OutPort);
...
RTC::ReturnCode_t ReaderES7::onInitialize()
{
  if (m_debug) {
    std::cerr << "ReaderES7::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t ReaderES7::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int ReaderES7::daq_dummy()
{
  return 0;
}

int ReaderES7::daq_configure()
{
  std::cerr << "*** ReaderES7::configure" << std::endl;
...
int ReaderES7::parse_params(::NVList* list)
{
...
int ReaderES7::daq_unconfigure()
{
  std::cerr << "*** ReaderES7::unconfigure" << std::endl;
...
int ReaderES7::daq_start()
{
  std::cerr << "*** ReaderES7::start" << std::endl;
...
int ReaderES7::daq_stop()
{
  std::cerr << "*** ReaderES7::stop" << std::endl;
...
int ReaderES7::daq_pause()
{
  std::cerr << "*** ReaderES7::pause" << std::endl;

  return 0;
}

int ReaderES7::daq_resume()
{
  std::cerr << "*** ReaderES7::resume" << std::endl;

  return 0;
}

int ReaderES7::read_data_from_detectors()
{
...
int ReaderES7::set_data(unsigned int data_byte_size)
{
...
int ReaderES7::daq_run()
{
  if (m_debug) {
    std::cerr << "*** ReaderES7::run" << std::endl;
  }
...
extern "C"
{
  void ReaderES7Init(RTC::Manager* manager)
  {
    ...
    manager->registerFactory(profile,
			     RTC::Create<ReaderES7>,
			     RTC::Delete<ReaderES7>);
  }
};
