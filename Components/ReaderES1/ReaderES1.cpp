...
static const char* reader_spec[] =
  {
   "implementation_id", "ReaderES1",
   "type_name",         "ReaderES1",
   "description",       "ReaderES1 component",
   "version",           "1.0",
...
ReaderES1::ReaderES1(RTC::Manager* manager)
...
RTC::ReturnCode_t ReaderES1::onInitialize()
{
  if (m_debug) {
    std::cerr << "ReaderES1::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t ReaderES1::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int ReaderES1::daq_dummy()
{
  return 0;
}

int ReaderES1::daq_configure()
{
  std::cerr << "*** ReaderES1::configure" << std::endl;
...
int ReaderES1::parse_params(::NVList* list)
{
...
int ReaderES1::daq_unconfigure()
{
  std::cerr << "*** ReaderES1::unconfigure" << std::endl;
...
int ReaderES1::daq_start()
{
  std::cerr << "*** ReaderES1::start" << std::endl;
...
int ReaderES1::daq_stop()
{
  std::cerr << "*** ReaderES1::stop" << std::endl;
...
int ReaderES1::daq_pause()
{
  std::cerr << "*** ReaderES1::pause" << std::endl;

  return 0;
}

int ReaderES1::daq_resume()
{
  std::cerr << "*** ReaderES1::resume" << std::endl;

  return 0;
}

int ReaderES1::read_data_from_detectors()
{
...
int ReaderES1::set_data(unsigned int data_byte_size)
{
...
int ReaderES1::daq_run()
{
  if (m_debug) {
    std::cerr << "*** ReaderES1::run" << std::endl;
  }
...
extern "C"
{
  void ReaderES1Init(RTC::Manager* manager)
  {
    ...
    manager->registerFactory(profile,
			     RTC::Create<ReaderES1>,
			     RTC::Delete<ReaderES1>);
  }
};
