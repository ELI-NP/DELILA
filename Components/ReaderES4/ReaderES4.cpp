...
static const char* reader_spec[] =
  {
   "implementation_id", "ReaderES4",
   "type_name",         "ReaderES4",
   "description",       "ReaderES4 component",
   "version",           "1.0",
...
ReaderES4::ReaderES4(RTC::Manager* manager)
    ...
    m_debug(false)
{
  ...
  // Set OutPort buffers
  registerOutPort("reader_es4_out", m_OutPort);
...
RTC::ReturnCode_t ReaderES4::onInitialize()
{
  if (m_debug) {
    std::cerr << "ReaderES4::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t ReaderES4::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int ReaderES4::daq_dummy()
{
  return 0;
}

int ReaderES4::daq_configure()
{
  std::cerr << "*** ReaderES4::configure" << std::endl;
...
int ReaderES4::parse_params(::NVList* list)
{
...
int ReaderES4::daq_unconfigure()
{
  std::cerr << "*** ReaderES4::unconfigure" << std::endl;
...
int ReaderES4::daq_start()
{
  std::cerr << "*** ReaderES4::start" << std::endl;
...
int ReaderES4::daq_stop()
{
  std::cerr << "*** ReaderES4::stop" << std::endl;
...
int ReaderES4::daq_pause()
{
  std::cerr << "*** ReaderES4::pause" << std::endl;

  return 0;
}

int ReaderES4::daq_resume()
{
  std::cerr << "*** ReaderES4::resume" << std::endl;

  return 0;
}

int ReaderES4::read_data_from_detectors()
{
...
int ReaderES4::set_data(unsigned int data_byte_size)
{
...
int ReaderES4::daq_run()
{
  if (m_debug) {
    std::cerr << "*** ReaderES4::run" << std::endl;
  }
...
extern "C"
{
  void ReaderES4Init(RTC::Manager* manager)
  {
    RTC::Properties profile(reader_spec);
    manager->registerFactory(profile,
			     RTC::Create<ReaderES4>,
			     RTC::Delete<ReaderES4>);
  }
};
