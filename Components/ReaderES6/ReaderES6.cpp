...
static const char* reader_spec[] =
  {
   "implementation_id", "ReaderES6",
   "type_name",         "ReaderES6",
   "description",       "ReaderES6 component",
   "version",           "1.0",
...
ReaderES6::ReaderES6(RTC::Manager* manager)
    ...
    m_debug(false)
{
  ...
  // Set OutPort buffers
  registerOutPort("reader_es6_out", m_OutPort);
...
RTC::ReturnCode_t ReaderES6::onInitialize()
{
  if (m_debug) {
    std::cerr << "ReaderES6::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t ReaderES6::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int ReaderES6::daq_dummy()
{
  return 0;
}

int ReaderES6::daq_configure()
{
  std::cerr << "*** ReaderES6::configure" << std::endl;
...
int ReaderES6::parse_params(::NVList* list)
{
...
int ReaderES6::daq_unconfigure()
{
  std::cerr << "*** ReaderES6::unconfigure" << std::endl;
...
int ReaderES6::daq_start()
{
  std::cerr << "*** ReaderES6::start" << std::endl;
...
int ReaderES6::daq_stop()
{
  std::cerr << "*** ReaderES6::stop" << std::endl;
...
int ReaderES6::daq_pause()
{
  std::cerr << "*** ReaderES6::pause" << std::endl;

  return 0;
}

int ReaderES6::daq_resume()
{
  std::cerr << "*** ReaderES6::resume" << std::endl;

  return 0;
}

int ReaderES6::read_data_from_detectors()
{
...
int ReaderES6::set_data(unsigned int data_byte_size)
{
...
int ReaderES6::daq_run()
{
  if (m_debug) {
    std::cerr << "*** ReaderES6::run" << std::endl;
  }
...
extern "C"
{
  void ReaderES6Init(RTC::Manager* manager)
  {
    RTC::Properties profile(reader_spec);
    manager->registerFactory(profile,
			     RTC::Create<ReaderES6>,
			     RTC::Delete<ReaderES6>);
  }
};
