...
Slave::Slave(RTC::Manager* manager)
...
RTC::ReturnCode_t Slave::onInitialize()
{
  if (m_debug) {
    std::cerr << "Slave::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t Slave::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int Slave::daq_configure()
{
  std::cerr << "*** Slave::configure" << std::endl;
...
int Slave::parse_params(::NVList* list)
{
...
int Slave::daq_unconfigure()
{
  std::cerr << "*** Slave::unconfigure" << std::endl;
...
int Slave::daq_pause()
{
  std::cerr << "*** Slave::pause" << std::endl;

  return 0;
}

int Slave::daq_resume()
{
  std::cerr << "*** Slave::resume" << std::endl;

  return 0;
}

int Slave::read_data_from_detectors()
{
...
int Slave::set_data(unsigned int data_byte_size)
{
...
extern "C"
{
  void SlaveInit(RTC::Manager* manager)
  {
