...
unsigned int Dispatcher::read_InPort() {
  ...
  constexpr auto headerFooterSize = HEADER_BYTE_SIZE + FOOTER_BYTE_SIZE;
  ...
  // For all ports
  if (m_debug) {
    std::cerr << "total size: " << dataSize1 + dataSize2 << std::endl;
  }
  ...
  // std::cout << m_in_status <<"\t"<< dataSize1 <<"\t"<< dataSize2 <<
  // std::endl;
