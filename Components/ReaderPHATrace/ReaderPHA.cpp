...
void ReaderPHA::DataReadThread()
{
  while (fDataProcessThreadFlag) {
    ...
    if (data->size() > 0) {
      std::unique_ptr<std::vector<std::unique_ptr<TraceData_t>>> localData(
          new std::vector<std::unique_ptr<TraceData_t>>);
      localData->reserve(data->size());
      for (auto i = 0; i < data->size(); i++) {
        std::unique_ptr<TraceData_t> traceData;
        traceData->Mod = data->at(i)->Mod;
        traceData->Ch = data->at(i)->Ch;
        traceData->FineTS = data->at(i)->FineTS;
        traceData->ADC = data->at(i)->ChargeLong;
        traceData->RecordLength = data->at(i)->RecordLength;
        traceData->Trace1.resize(traceData->RecordLength);
        for (auto j = 0; j < traceData->RecordLength; j++) {
          traceData->Trace1[j] = data->at(i)->Trace1[j];
        }
        localData->push_back(std::move(traceData));
      }
      {
        std::lock_guard<std::mutex> lock(fDataMutex);
        fDataVec.push_back(std::move(localData));
      }
    }
    usleep(10000);
  }
...
void ReaderPHA::DataProcessThread()
{
  ...
  while (fDataProcessThreadFlag) {
    ...
    if (dataSize > 0) {
      std::vector<std::unique_ptr<std::vector<std::unique_ptr<TraceData_t>>>>
          dataVec;
      {
        std::lock_guard<std::mutex> lock(fDataMutex);
      ...
      for (auto i = 0; i < dataVec.size(); i++) {
        for (auto j = 0; j < dataVec[i]->size(); j++) {
          ...
          const auto oneHitSize =
              sizeMod + sizeCh + sizeFineTS + sizeADC + sizeRL +
              (sizeof(TraceData::Trace1[0]) * data->RecordLength);
          auto index = 0;
          std::vector<char> hit;
          ...
          memcpy(&hit[index], &(data->ADC), sizeADC);
          index += sizeADC;
