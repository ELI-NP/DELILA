...
static const char* reader_spec[] =
  {
   "implementation_id", "ReaderES3",
   "type_name",         "ReaderES3",
   "description",       "ReaderES3 component",
   "version",           "1.0",
...
ReaderES3::ReaderES3(RTC::Manager* manager)
    ...
    m_debug(false)
{
  ...
  // Set OutPort buffers
  registerOutPort("reader_es3_out", m_OutPort);
...
RTC::ReturnCode_t ReaderES3::onInitialize()
{
  if (m_debug) {
    std::cerr << "ReaderES3::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t ReaderES3::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int ReaderES3::daq_dummy()
{
  return 0;
}

int ReaderES3::daq_configure()
{
  std::cerr << "*** ReaderES3::configure" << std::endl;
...
int ReaderES3::parse_params(::NVList* list)
{
...
int ReaderES3::daq_unconfigure()
{
  std::cerr << "*** ReaderES3::unconfigure" << std::endl;
...
int ReaderES3::daq_start()
{
  std::cerr << "*** ReaderES3::start" << std::endl;
...
int ReaderES3::daq_stop()
{
  std::cerr << "*** ReaderES3::stop" << std::endl;
...
int ReaderES3::daq_pause()
{
  std::cerr << "*** ReaderES3::pause" << std::endl;

  return 0;
}

int ReaderES3::daq_resume()
{
  std::cerr << "*** ReaderES3::resume" << std::endl;

  return 0;
}

int ReaderES3::read_data_from_detectors()
{
...
int ReaderES3::set_data(unsigned int data_byte_size)
{
...
int ReaderES3::daq_run()
{
  if (m_debug) {
    std::cerr << "*** ReaderES3::run" << std::endl;
  }
...
extern "C"
{
  void ReaderES3Init(RTC::Manager* manager)
  {
    RTC::Properties profile(reader_spec);
    manager->registerFactory(profile,
			     RTC::Create<ReaderES3>,
			     RTC::Delete<ReaderES3>);
  }
};
