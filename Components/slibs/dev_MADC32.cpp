...
// g++ -shared -o dev_MADC32.so dev_MADC32.o
// /home/gant/DELILA-main/Components/src/VMEController.o
// /home/gant/DELILA-main/Components/src/VMEDevice.o -ldl
...
public:
  ...
  void read_cfg();
  std::unique_ptr<VMEController>
  mod_configure(std::unique_ptr<VMEController> my_contr, uint32_t addr_offset,
                int modId);
  std::unique_ptr<VMEController>
  mod_start(std::unique_ptr<VMEController> my_contr);
  std::unique_ptr<VMEController>
  mod_stop(std::unique_ptr<VMEController> my_contr);
  std::unique_ptr<VMEController>
  mod_run(std::unique_ptr<VMEController> my_contr,
          std::shared_ptr<std::vector<TreeData>> t_data, int *fNEvents);
  int mod_checkifreader();
};

void MADC32::read_cfg() {

  std::string disk_loc_begin =
      "/home/gant/DELILA-main/Components/slibs/conf_MADC32_0x";
  ...
  if (has_exception == false) {
    ...
    this->max_data_transf =
        stoi(conf_data["max_data_transf"].get<std::string>());
    ...
    std::cout << "mod_nr=" << this->mod_nr << " irql=" << this->irq_l
              << " irqdt=" << this->irq_data_thr
              << " mdt=" << this->max_data_transf << " ets=" << this->ext_ts
              << " ro=" << this->res_opt << " ir=" << this->in_range
              << " hd0=" << this->hold_d0 << " hw0=" << this->hold_w0
              << " hd1=" << this->hold_d1 << " hw1=" << this->hold_w1
              << " bop=" << this->bank_op << " nimb=" << nim_busy
              << " usegg=" << use_gg << std::endl;
...
std::unique_ptr<VMEController>
MADC32::mod_configure(std::unique_ptr<VMEController> my_contr,
                      uint32_t addr_offset, int modId) {

  std::cout << "\nConfigure for mod " << modId << " with address " << std::hex
            << addr_offset << " begin" << std::endl;
...
std::unique_ptr<VMEController>
MADC32::mod_start(std::unique_ptr<VMEController> my_contr) {
...
std::unique_ptr<VMEController>
MADC32::mod_stop(std::unique_ptr<VMEController> my_contr) {
...
std::unique_ptr<VMEController>
MADC32::mod_run(std::unique_ptr<VMEController> my_contr,
                std::shared_ptr<std::vector<TreeData>> t_data, int *fNEvents) {
    ...
    if (ext_ts == 0) {
      ...
      for (int i = 0; i < buffSize; i++) {
        ...
        if ((dataBuff[i] >> 30) == 1) {
          ...
          if ((dataBuff[i] >> 22) == 16) {
            ...
            // std::cerr<<"The event data is:
            // "<<std::bitset<32>(dataBuff[i])<<std::endl;
      ...
      for (int i = 0; i < buffSize; i++) {
        ...
        if ((dataBuff[i] >> 30) == 1) {
          ...
          // first 2 bits are always 11 for EOE mark; we only use 30 for the
          // time stamp
...
int MADC32::mod_checkifreader() { return 1; }
