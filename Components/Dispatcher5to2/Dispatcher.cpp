...
unsigned int Dispatcher::read_InPort()
{
   ...
   if (m_debug) {
      std::cerr << "m_in3_data.data.length():" << dataSize3
                << std::endl;
   }
   ...
   if (m_debug) {
      std::cerr << "m_in4_data.data.length():" << dataSize4
                << std::endl;
   }
   ...
   if (m_debug) {
      std::cerr << "m_in5_data.data.length():" << dataSize5
                << std::endl;
   }


   // For all ports
   auto totalDataSize = dataSize1 + dataSize2 + dataSize3 + dataSize4 + dataSize5;
   ...
   if(totalDataSize > 0){
	...
	dataIndex += dataSize2;
      ...
      if(dataSize3 > 0) {
	memcpy(&(m_in_data.data[dataIndex]), &m_in3_data.data[HEADER_BYTE_SIZE], dataSize3);
	dataIndex += dataSize3;
      }
      if(dataSize4 > 0) {
	memcpy(&(m_in_data.data[dataIndex]), &m_in4_data.data[HEADER_BYTE_SIZE], dataSize4);
	dataIndex += dataSize4;
      }
      if(dataSize5 > 0) {
	memcpy(&(m_in_data.data[dataIndex]), &m_in5_data.data[HEADER_BYTE_SIZE], dataSize5);
	dataIndex += dataSize5;
      }
