      ...
      m_debug(false)
{
  ...
  fNInPorts = 5;
  fDataVec.resize(fNInPorts);
  fInPortVec.resize(fNInPorts);
  for (auto iPort = 0; iPort < fNInPorts; iPort++) {
    auto portName = "dispatcher_in" + std::to_string(iPort);
    fInPortVec[iPort](portName.c_str(), fDataVec[iPort]);
  }
...
unsigned int Dispatcher::read_InPort()
{
  ...
  // For all ports
  if (m_debug) {
    std::cerr << "total size: " << dataSize1 + dataSize2 + dataSize3
              << std::endl;
  }
