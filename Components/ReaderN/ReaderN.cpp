...
ReaderN::ReaderN(RTC::Manager *manager)
...
RTC::ReturnCode_t ReaderN::onInitialize()
{
  if (m_debug) {
    std::cerr << "ReaderN::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t ReaderN::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int ReaderN::daq_dummy() { return 0; }

int ReaderN::daq_configure()
{
  std::cerr << "*** ReaderN::configure" << std::endl;
...
int ReaderN::parse_params(::NVList *list)
{
...
int ReaderN::daq_unconfigure()
{
  std::cerr << "*** ReaderN::unconfigure" << std::endl;
...
int ReaderN::daq_start()
{
  std::cerr << "*** ReaderN::start" << std::endl;
...
int ReaderN::daq_stop()
{
  std::cerr << "*** ReaderN::stop" << std::endl;
...
int ReaderN::daq_pause()
{
  std::cerr << "*** ReaderN::pause" << std::endl;

  return 0;
}

int ReaderN::daq_resume()
{
  std::cerr << "*** ReaderN::resume" << std::endl;

  return 0;
}

void ReaderN::StartThreads()
{
  fDataReadThreadFlag = true;
  fDataReadThread = std::thread(&ReaderN::DataReadThread, this);

  fDataProcessThreadFlag = true;
  fDataProcessThread = std::thread(&ReaderN::DataProcessThread, this);
}

void ReaderN::StopThreads()
{
...
void ReaderN::DataProcessThread()
{
...
extern "C" {
void ReaderNInit(RTC::Manager *manager)
{
  RTC::Properties profile(reader_spec);
  manager->registerFactory(profile, RTC::Create<ReaderN>,
                           RTC::Delete<ReaderN>);
}
};
