...
class ReaderScope2 : public DAQMW::DaqComponentBase
{
 public:
  ReaderScope2(RTC::Manager *manager);
  ~ReaderScope2();
 ...
 private:
  ...
  // Digitizer
  std::vector<std::unique_ptr<Scope2>> fDigitizer;
  ...
  std::vector<std::vector<std::unique_ptr<Scope2Data>>> fRawData;
  // std::vector<std::unique_ptr<Scope2Data>> fDataVec;
  std::vector<std::mutex> fRawDataMutex;
  void StartThreads();
  void StopThreads();
